...
# NTHU dataset (no annotations) > shape_predictor_68_face_landmarks.dat --> model of dlib's face predictor >
# eyeblink8 folder --> 8 videos with annotations for blink detection purposes made by
# https://www.blinkingmatters.com/research > talkingFace folder --> 1 video with annotation for blink detection
# purposes made by https://www.blinkingmatters.com/research
#
...
def read_frame_from_serial():
    """Read a single frame (JPEG) from the Serial connection."""
    frame_data = b''
    in_image = False  # Flag to track if we are inside an image data stream
    ...
    while True:
        ...
        # If not currently in an image, check for the start marker
        if not in_image:
            if byte == b'\xFF':  # Possible start of JPEG
                next_byte = ser.read(1)
                if next_byte == b'\xD8':  # Confirm JPEG start marker
                    frame_data = b'\xFF\xD8'
                    in_image = True
            continue

        # If inside an image, collect data
        frame_data += byte

        # Check for the JPEG end marker
...
def process_video(input_file, detector=dlib_detector, predictor=dlib_predictor, lStart=42, lEnd=48, rStart=36, rEnd=42,
                  ear_th=0.21, consec_th=3, up_to=None):
...
frame_info_df, closeness_predictions_skipped, blink_predictions_skipped, frames_skipped = skip_first_n_frames(
    frame_info_df, closeness_predictions, blink_predictions, frames, skip_n=SKIP_FIRST_FRAMES)
...
def display_blinks(blinks, processed_frames):
    ...
    for (frame_start, frame_end) in blinks:
        ...
        print("{}th blink started at: {}th frame, middle of action at: {}th frame, ended at: {}th frame".format(i,
                                                                                                                frame_start,
                                                                                                                frame_middle,
                                                                                                                frame_end))
        i += 1
...
def write_outputs(input_file_name, closeness_list, blinks_list, frame_info_df=None, scores=None, test=True,
                  scores_only=True):
...
def load_datasets(path, dataset_name):
    ...
    # read prediction results and frame_info_df
    closeness_pred, blinks_pred, frame_info_df = read_outputs("{}_pred".format(full_path),
                                                              "{}_frame_info_df".format(full_path))
    # print(read_outputs)
...
def simple_model(input_full_path, ear_th=0.21, consec_th=3, skip_n=0, display_blinks=True, test_extention=False,
                 write_results=True):
    # define variables
    scores_string = ""
    ...
    if write_results == True:
        # write prediction output files by using outputs of skip_first_n_frames() function
        write_outputs(input_full_path, closeness_predictions_skipped, blink_predictions_skipped, frame_info_df,
                      scores_string)
        if test_extention != False:
            # write test output files by using outputs of skip_first_n_frames() function
            # no need to write frame_info_df and scores_string since they already have written above
...
fdf, cp, bp, fr, vi, st = simple_model("E:/Eye-Blink-Detection-master/input/talkingFace/talking.avi",
